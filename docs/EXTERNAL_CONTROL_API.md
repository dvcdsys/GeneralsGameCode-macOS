...
| `/units?player=<i>&view=<j>` | GET | objects: `id, template, player, x/y/z, health, maxHealth, relationToLocal, category, tags[]`, plus dynamic state when non-default: `veterancy` (veteran/elite/heroic), `experience`, `visionRange`, `contains` (passengers). `view=j` applies **synthesized fog-of-war** for player j via the `shroud` field — `clear` (in sight, live state), `cached` (building seen before, now out of sight: last-known snapshot), `undefined` (neutral landmark known from the map but never seen: position only, no state). Hidden objects (un-scouted enemies, units out of sight) are omitted. `player=i` filters by owner. |
